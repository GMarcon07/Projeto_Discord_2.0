...
export const ChatArea: React.FC<ChatAreaProps> = ({ onSendMessage }) => {
  ...
  if (!channel) {
    return (
      <main className="flex-1 bg-[#313338] flex items-center justify-center text-[#949ba4] text-sm">
  ...
  return (
    <main className="flex-1 bg-[#313338] flex flex-col min-w-0 h-full">
      {/* Channel Header */}
      <div className="h-12 border-b border-[#1f2023] px-4 flex items-center gap-2 font-semibold text-white shadow-sm shrink-0">
        <Hash size={22} className="text-[#80848e]" />
        <span className="text-sm">{channel.name}</span>
        <div className="h-4 w-[1px] bg-[#3f4147] mx-2" />
        <span className="text-xs text-[#949ba4] font-normal truncate">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        <div className="pt-8 pb-4 flex flex-col gap-2">
          ...
          <h3 className="text-2xl font-bold text-white">Bem-vindo a #{channel.name}!</h3>
        ...
        {channelMessages.map((msg, index) => {
          ...
          if (isCompact) {
            return (
              <div key={msg.id} className="group flex pl-14 py-0.5 hover:bg-[#2e3035] -mx-4 px-4 rounded transition-colors relative">
                <span className="absolute left-4 opacity-0 group-hover:opacity-100 text-[10px] text-[#949ba4] select-none pt-0.5">
                  {formatMessageTime(msg.createdAt)}
                </span>
                <p className="text-sm text-[#dbdee1] leading-relaxed break-words whitespace-pre-wrap select-text">
                  {msg.content}
                </p>
              </div>
            );
          ...
          return (
            <div key={msg.id} className="flex gap-3 hover:bg-[#2e3035] -mx-4 px-4 py-1.5 rounded transition-colors">
              {/* Avatar */}
              <div
              ...
              <div className="flex flex-col min-w-0 flex-1">
                ...
                <p className="text-sm text-[#dbdee1] leading-relaxed break-words whitespace-pre-wrap select-text mt-0.5">
                  {msg.content}
                </p>
              </div>
            </div>
      ...
      <div className="px-4 pb-5 pt-1">
        <form onSubmit={handleSubmit} className="bg-[#383a40] rounded-lg flex items-center px-4 py-2.5 gap-3 shadow-inner">
          <textarea
            value={inputText}
            ...
            className="bg-transparent flex-1 text-sm text-white placeholder-[#80848e] focus:outline-none resize-none max-h-32 select-text"
          />
          <button
            type="submit"
            disabled={!inputText.trim()}
            className="text-[#5865F2] hover:text-white disabled:opacity-30 transition-colors p-1"
